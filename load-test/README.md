...
`GatewayLoadTest`는 gateway 한 JVM과 Embedded Kafka, WireMock을 사용합니다.
운영 규모를 재현하는 시험은 아니지만 메시지 손실과 전달 지연, 라우팅 처리량을 빠르게
확인할 수 있습니다. 포트폴리오 hardening 이후 현재 소스로는 다시 측정하지
않았으므로 날짜별 저장 결과는 현 릴리스의 성능 수치가 아닙니다.
...
현재 릴리스의 측정 상태와 hardening 이전 자료의 범위는
