 ...
 * connect timeout, a fixed command timeout, and rejection of commands while disconnected keep that
 * degraded path bounded. Automatic reconnect remains enabled so distributed limiting resumes when
 * Redis recovers.
 */
@Configuration
...
public class RateLimitConfig {
  ...
  RedisClient rateLimitRedisClient(RedisProperties redis) {
    ...
    RedisURI.Builder uri =
        RedisURI.builder().withHost(host).withPort(redis.getPort()).withTimeout(COMMAND_TIMEOUT);
